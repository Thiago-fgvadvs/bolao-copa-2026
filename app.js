...
function App() {
  ...
  const isLocked = useCallback((game) => {
    if (game.round <= 3) { // fase de grupos: ABERTO até a trava manual dos organizadores (Dr. Thiago / Eduardo)
      return !!cfg.gruposTravado;
    }
  ...
  }, [now, cfg.gruposTravado, cfg.mataAbre, cfg.deadlineMata]);
  ...
  return (
    <div style={{ background: C.bg, color: C.txt, minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div className="p-3 pb-24 max-w-2xl mx-auto">
        ...
        {screen === "jogos" && <Jogos games={games} myGuesses={myGuesses} setGuess={setGuess} isLocked={isLocked} results={results} cfg={cfg} now={now} />}
...
// ---------- Palpites ----------
function Jogos({ games, myGuesses, setGuess, isLocked, results, cfg, now }) {
